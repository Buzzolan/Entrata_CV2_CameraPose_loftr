...
indexImg='1020'# 159=CV2, 1020=Dante, 2=Fontana, 67=Targa
indexTest='2'# da 1 a 10
env='dante'#dante, Fontana, CV2, Targa
img0_pth= env+"/proof/_SAM"+indexImg+".JPG"#0=CV2, 00=Targa, 000=Fontana
img1_pth= env+"/proof/test_"+indexTest+".jpg"
fs1=0.18
fs2=0.18
...
plt.title("RefImg Punti 2D con corrispondenza 3D")
plt.imshow(RefImg,cmap='Greys_r')
plt.savefig(env+"/foto_proof/Test_"+indexTest+"_Punti_2D_con_corrispondenza_3D.jpg") #save as jpg
...
#Load two image and show it: controllo che siano orientate correttamente altrimenti loftr fa un casino 
# one image is fixed because in the pipeline i have the 2D-3D correspondence



#TestImg=cv2.rotate(cv2.cvtColor(image.imread(img1_pth),cv2.COLOR_BGR2GRAY), cv2.cv2.ROTATE_180)
#TestImg=cv2.rotate(cv2.cvtColor(image.imread(img1_pth),cv2.COLOR_BGR2GRAY), cv2.cv2.ROTATE_180_CLOCKWISE)
...
plt.imshow(SubRef_img0,cmap='Greys_r')
plt.savefig(env+"/foto_proof/Test_"+indexTest+"Punti_salienti_loftr.jpg") #save as jpg
...
plt.title("Sovrapposizione dei punti salienti trovati con loftr ed i punti 2D-3D")
plt.imshow(SubRef_img0,cmap='Greys_r')
plt.savefig(env+"/foto_proof/Test_"+indexTest+"_loftr_2D_3D.jpg") #save as jpg
c=0
...
plt.savefig(env+"/foto_proof/Test_"+indexTest+"_Visione_Generale.jpg") 
...
plt.imshow(SubTest_img1)
plt.savefig(env+"/foto_proof/Test_"+indexTest+"_CorrsipondenzeLoftr_3D.jpg") 
...
scipy.io.savemat(env+"/risultati_proof/Loftr_points"+indexTest+".mat", Dict_corrispondenze_test)
scipy.io.savemat(env+"/risultati_proof/Loftr_pointsRef"+indexImg+".mat", Dict_corrispondenze_Ref)
